...
The question we decided to answer is if there is any difference between home run distance among the American league compared to the National League of the MLB (Major League Baseball). The value to answering this question is to see which league, if either, has a statical advantage when it comes to distance of home runs. This is important because each field has varying distances for a home run, which would give a competitive edge to the league that, on average, can hit farther home runs, resulting in a higher amount of hits actually being home runs.

## Methods

Methods: Explain how the data were obtained, as well as providing a citation of their source. 
*The data were obtained from ESPN’s site hittrackeronline.com (URL: http://www.hittrackeronline.com/top_true_distance.php). This site is ran by ESPN Stats and Information Group. It contains a list of all of the home runs hit in the 2017 season including: who hit the home run, what team they play for, and the true distance of how far the ball was hit measured in feet.*
...
```{r, eval=TRUE, echo=FALSE}
plot(density(vars_AL))
lines(density(vars_NL))
```
...
Include properly formatted and informative graphic(s) for this function of parameters. 
```{r, eval=TRUE, echo=FALSE}
plot(density(diffs))
```
